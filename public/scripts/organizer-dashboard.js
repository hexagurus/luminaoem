document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Fetch Events
    try {
        const res = await apiAuthRequest('/api/organizer/events');
    ...
    window.deleteEvent = async (id) => {
        ...
        try {
            const res = await apiAuthRequest(`/api/events/${id}`, {
                method: 'DELETE'
            });
    ...
    async function onScanSuccess(decodedText, decodedResult) {
        ...
        try {
            const res = await apiAuthRequest('/api/checkin/validate', {
                method: 'POST',
    ...
    if (user && user.id) {
        // Connect to Socket.IO using the centralized BASE_URL
