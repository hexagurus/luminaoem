document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function fetchAnalytics() {
        try {
            const res = await apiAuthRequest('/api/bookings/analytics/user');
    ...
    async function fetchBookings() {
        try {
            ...
            const res = await apiAuthRequest(`/api/bookings/my-bookings?${params.toString()}`);
...
async function downloadTicket(bookingId) {
    try {
        const res = await apiAuthRequest(`/api/bookings/${bookingId}/download-ticket`);
...
async function cancelBooking(bookingId) {
    ...
    try {
        const res = await apiAuthRequest(`/api/bookings/${bookingId}/cancel`, {
            method: 'PUT'
        });
        const data = await res.json();
...
async function requestRefund(bookingId) {
    ...
    try {
        const res = await apiAuthRequest(`/api/bookings/${bookingId}/request-refund`, {
            method: 'POST'
        });
        const data = await res.json();
