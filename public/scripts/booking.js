document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (applyBtn) {
        applyBtn.addEventListener('click', async () => {
            ...
            try {
                const res = await apiAuthRequest('/api/coupons/validate', {
                    method: 'POST',
    ...
    if (bookingForm) {
        bookingForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                const orderRes = await apiAuthRequest('/api/payments/orders', {
                    method: 'POST',
                ...
                // 2. Open Razorpay
                const keyRes = await apiAuthRequest('/api/payments/key');
                ...
                const options = {
                    ...
                    "handler": async function (response) {
                        // 3. Verify Payment
                        const verifyRes = await apiAuthRequest('/api/payments/verify', {
                            method: 'POST',
                            body: JSON.stringify({
                        ...
                        if (verifyRes.ok) {
                            ...
                            // 4. Create Booking
                            try {
                                await apiAuthRequest('/api/bookings/create', {
                                    method: 'POST',
                                    body: JSON.stringify({
                                        eventId: eventId,
