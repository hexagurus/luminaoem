...
// Dynamic env config for frontend — serves BASE_URL from .env to the browser
// Must be registered BEFORE the static file middleware so it takes precedence
app.get('/scripts/env-config.js', (req, res) => {
    const baseUrl = process.env.BASE_URL || `http://localhost:${PORT}`;
    res.setHeader('Content-Type', 'application/javascript');
    res.send(`window.__ENV__ = { BASE_URL: '${baseUrl}' };`);
});
